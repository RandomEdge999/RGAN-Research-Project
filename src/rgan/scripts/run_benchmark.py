...
from rgan.tslib_data import (
    TSLIB_DATASETS,
    load_dataset,
    split_and_standardize,
)
from rgan.data import make_windows_univariate
from rgan.config import TrainConfig
from rgan.models_torch import build_generator, build_discriminator
from rgan.rgan_torch import train_rgan_torch
from rgan.lstm_supervised_torch import train_lstm_supervised_torch
from rgan.baselines import naive_baseline, arima_forecast, tree_ensemble_forecast
...
from rgan.metrics import error_stats

# Project root for default data/results paths
...
def run_single(
    ...
    # ── DLinear ───────────────────────────────────────────────────
    t0 = time.time()
    dl_out = train_linear_baseline(config, data_dict, str(sub_dir), model_type="dlinear", tag="DLinear")
    dl_pred = dl_out["pred_test"]
    dl_stats = error_stats(Y_test.reshape(-1), dl_pred.reshape(-1))
    results["DLinear"] = {"mse": dl_stats["mse"], "mae": dl_stats["mae"]}
    print(f"  DLinear   MSE={dl_stats['mse']:.6f}  MAE={dl_stats['mae']:.6f}  ({time.time()-t0:.0f}s)")

    # ── NLinear ───────────────────────────────────────────────────
    t0 = time.time()
    nl_out = train_linear_baseline(config, data_dict, str(sub_dir), model_type="nlinear", tag="NLinear")
    nl_pred = nl_out["pred_test"]
    nl_stats = error_stats(Y_test.reshape(-1), nl_pred.reshape(-1))
    results["NLinear"] = {"mse": nl_stats["mse"], "mae": nl_stats["mae"]}
    print(f"  NLinear   MSE={nl_stats['mse']:.6f}  MAE={nl_stats['mae']:.6f}  ({time.time()-t0:.0f}s)")

    # ── FITS ─────────────────────────────────────────────────────
    t0 = time.time()
    fits_out = train_fits(config, data_dict, str(sub_dir), tag="FITS")
    fits_pred = fits_out["pred_test"]
    fits_stats = error_stats(Y_test.reshape(-1), fits_pred.reshape(-1))
    results["FITS"] = {"mse": fits_stats["mse"], "mae": fits_stats["mae"]}
    print(f"  FITS      MSE={fits_stats['mse']:.6f}  MAE={fits_stats['mae']:.6f}  ({time.time()-t0:.0f}s)")

    # ── PatchTST ─────────────────────────────────────────────────
    t0 = time.time()
    ptst_out = train_patchtst(config, data_dict, str(sub_dir), tag="PatchTST")
    ptst_pred = ptst_out["pred_test"]
    ptst_stats = error_stats(Y_test.reshape(-1), ptst_pred.reshape(-1))
    results["PatchTST"] = {"mse": ptst_stats["mse"], "mae": ptst_stats["mae"]}
    print(f"  PatchTST  MSE={ptst_stats['mse']:.6f}  MAE={ptst_stats['mae']:.6f}  ({time.time()-t0:.0f}s)")

    # ── iTransformer ─────────────────────────────────────────────
    t0 = time.time()
    itrans_out = train_itransformer(config, data_dict, str(sub_dir), tag="iTransformer")
    itrans_pred = itrans_out["pred_test"]
    itrans_stats = error_stats(Y_test.reshape(-1), itrans_pred.reshape(-1))
    results["iTransformer"] = {"mse": itrans_stats["mse"], "mae": itrans_stats["mae"]}
    print(f"  iTransfmr MSE={itrans_stats['mse']:.6f}  MAE={itrans_stats['mae']:.6f}  ({time.time()-t0:.0f}s)")
