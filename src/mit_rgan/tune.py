...
from .models_keras import build_generator as build_generator_tf, build_discriminator as build_discriminator_tf
from .rgan_keras import train_rgan_keras as train_rgan_tf

def tune_rgan_keras(hp_grid: Dict, base_config: Dict, data_splits: Dict, results_dir: str, backend: str = "tf"):
    ...
    for units_g in hp_grid.get("units_g", [64]):
        for units_d in hp_grid.get("units_d", [64]):
            for lambda_reg in hp_grid.get("lambda_reg", [0.1]):
                ...
                if backend == "torch":
                    import torch
                    from .models_torch import build_generator as build_generator_th, build_discriminator as build_discriminator_th
                    from .rgan_torch import train_rgan_torch as train_rgan_backend
                    G = build_generator_th(cfg["L"], cfg["H"], n_in=Xtr.shape[-1], units=units_g, dropout=cfg["dropout"])
                    D = build_discriminator_th(cfg["L"], cfg["H"], units=units_d, dropout=cfg["dropout"])
                    ...
                    G = build_generator_tf(cfg["L"], cfg["H"], n_in=Xtr.shape[-1], units=units_g, dropout=cfg["dropout"])
                    D = build_discriminator_tf(cfg["L"], cfg["H"], units=units_d, dropout=cfg["dropout"])
                    splits = dict(Xtr=Xtr, Ytr=Ytr, Xval=Xval, Yval=Yval, Xte=Xval, Yte=Yval)
                    out = train_rgan_tf(cfg, (G,D), splits, results_dir, tag=f"tune_g{units_g}_d{units_d}_l{lambda_reg}")
